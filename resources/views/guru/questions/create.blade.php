<x-app-layout>
    <x-slot name="header">
        <h2 class="font-fredoka font-semibold text-2xl text-emerald-700 leading-tight">
            {{ __('Buat Soal Kustom') }} (Sesi: {{ $session->title }})
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl p-8 shadow-md border border-gray-100">
                <form method="POST" action="{{ route('guru.sessions.questions.store', $session) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    <!-- Nama Sampah -->
                    <div>
                        <x-input-label for="nama_sampah" :value="__('Nama Sampah / Objek')" />
                        <x-text-input id="nama_sampah" class="block mt-1 w-full" type="text" name="nama_sampah" :value="old('nama_sampah')" required autofocus placeholder="Contoh: Kertas Struk, Cangkang Kerang" />
                    ...
                    <div>
                        ...
                        <select id="kategori" name="kategori" required class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm">
                            ...
                            <option value="anorganik" {{ old('kategori') === 'anorganik' ? 'selected' : '' }}>Anorganik (Kuning - Didaur Ulang)</option>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Disarankan gambar berformat PNG transparan/kartun dengan rasio 1:1 maksimal 2MB.</p>
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100">
                        <a href="{{ route('guru.sessions.show', $session) }}" class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-fredoka font-semibold text-sm rounded-2xl transition duration-150">
                            Batal
                        </a>
